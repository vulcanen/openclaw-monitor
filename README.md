...
- **Conversation audit** *(optional)*: captures the four touchpoints of a conversation — sender → OpenClaw → LLM → OpenClaw → sender — across multi-hop LLM calls; the list groups by `sessionKey` with collapsible panels, the detail view shows full prompts and responses (1 MiB per-segment cap by default)
